...
class RemoteSelector extends Component {
  ...
  onAddRemoveItem = value => {
    const { selectedValues, options } = this.state;
    const ind = selectedValues.findIndex(i => i._id === value._id);
    if (ind === -1) {
      ...
      this.setState(
        ...
        () => {
          this.logChange(newvals);
        }
      );
    } else {
      const newitems = selectedValues.filter(i => i._id !== value._id);
      this.setState(
        {
          options: options.map(opt => {
            if (opt._id === value._id) {
              const newopt = { ...opt };
              delete newopt.checked;
              return { ...newopt };
            }
            return opt;
          }),
